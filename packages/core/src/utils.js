 ...
 */
export async function createEnhancedSiteConfig(shopDomain, accessToken) {
 ...
 */
function createSiteConfigWithProducts(shopDomain, products) {
 ...
 */
function createBaseSiteConfig(shopDomain, productItems, featuredProduct) {
    return {
        id: generateConfigId(),
        version: "1.0.0",
        categories: [
                        ...
                        children: [
                            ...
                            },
                            {
                                ...
                                card: [
                                    ...
                                    {
                                        link: null,
                                        image: "https://images.unsplash.com/photo-1506629905587-4b1d7673dab7?w=800&h=600&fit=crop",
 ...
 */
export function createDefaultSiteConfig(shopDomain) {
...
export function reorderCategories(categories, fromIndex, toIndex) {
...
export function addToCart(currentItems, product, variantId, quantity = 1) {
    const variant = product.variants.find((v) => v.id === variantId);
    if (!variant) {
        throw new Error("Variant not found");
    }
    const existingItemIndex = currentItems.findIndex((item) => item.productId === product.id && item.variantId === variantId);
...
export function removeFromCart(currentItems, itemId) {
    return currentItems.filter((item) => item.id !== itemId);
}
export function updateCartItemQuantity(currentItems, itemId, quantity) {
    if (quantity <= 0) {
...
// Shopify utilities
export function buildCheckoutUrl(shopDomain, items) {
...
export function extractShopFromDomain(domain) {
...
export function buildStorefrontUrl(shopDomain) {
...
export function createPerformanceMetrics(configId) {
    if (typeof window === "undefined") {
...
// URL utilities
export function buildSiteUrl(baseUrl, configId, draft) {
...
export function buildProductUrl(baseUrl, configId, productId) {
...
export function generateImageSrcSet(url, sizes) {
    return sizes
