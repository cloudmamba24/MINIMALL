...
export class WebhookHandler {
	...
	private async handleAppUninstall(context: WebhookContext): Promise<void> {
		...
		try {
			...
			await db
				.delete(users)
				.where(eq(users.shopDomain, shop));
			...
			await db
				.delete(featureFlags)
				.where(eq(featureFlags.shopDomain, shop));
	...
	private async handleCustomerDataRequest(
		context: WebhookContext,
	): Promise<void> {
	...
	private async handleCustomerRedact(context: WebhookContext): Promise<void> {
		...
		console.log(
			`Customer redaction request for shop ${shop}, customer: ${customerId}`,
		);
	...
	private async handleShopRedact(context: WebhookContext): Promise<void> {
		...
		try {
			...
			await db
				.delete(webhooks)
				.where(eq(webhooks.shopDomain, shop));
...
export function getWebhookHandler(): WebhookHandler {
	if (!webhookHandlerInstance) {
		...
		if (!secretKey) {
			throw new Error(
				"SHOPIFY_API_SECRET environment variable is required for webhook handling",
			);
		}
