 ...
 */
async function reportWebVital(metric, configId) {
    try {
        ...
        if (metric.rating === "poor") {
            Sentry.captureMessage(`Poor Web Vital: ${metric.name} = ${Math.round(metric.value)}ms`, "warning");
        }
    }
    catch (error) {
        console.warn("Failed to report web vital:", error);
 ...
 */
export function initPerformanceMonitoring(configId) {
...
export class PerformanceTracker {
    ...
    async reportCustomMetric(name, duration) {
        try {
...
export function usePerformanceTracking() {
    ...
    const trackAsyncOperation = async (name, operation) => {
