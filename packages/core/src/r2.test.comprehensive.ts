...
describe("R2 Storage Service - Comprehensive Tests", () => {
	...
	describe("R2ConfigService Class", () => {
		...
		it("should have proper error message for missing variables", () => {
			...
			expect(() => new R2ConfigService())
				.toThrow("Missing required R2 environment variables. Required: R2_ENDPOINT, R2_ACCESS_KEY, R2_SECRET, R2_BUCKET_NAME");
		});
	});
	...
	describe("File Upload Operations", () => {
		describe("putObject via R2ConfigService", () => {
			...
			it("should handle upload errors", async () => {
				...
				await expect(service.putObject("test.jpg", "content"))
					.rejects.toThrow("Failed to upload to R2: 500 Internal Server Error");
			});
		...
		describe("Configuration Operations", () => {
			...
			describe("saveConfig", () => {
				...
				it("should handle save errors", async () => {
					...
					await expect(service.saveConfig("test-config", mockSiteConfig))
						.rejects.toThrow("Failed to upload to R2: 500 Internal Server Error");
				});
			});
			...
			describe("getConfig", () => {
				...
				it("should handle configuration not found", async () => {
					...
					await expect(service.getConfig("nonexistent-config"))
						.rejects.toThrow("Configuration not found: nonexistent-config");
				});
			});
		...
		describe("Error Handling and Edge Cases", () => {
			it("should handle network failures gracefully", async () => {
				...
				await expect(service.getConfig("test-config"))
