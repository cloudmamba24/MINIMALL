...
vi.mock('@shopify/app-bridge-react', () => ({
  ...
  TitleBar: ({ title }: { title: string }) => React.createElement('div', { 'data-testid': 'title-bar' }, title),
  Loading: ({ loading }: { loading: boolean }) => loading ? React.createElement('div', { 'data-testid': 'loading' }) : null,
}));
