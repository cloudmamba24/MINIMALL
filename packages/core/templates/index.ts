...
export const CORE_TEMPLATES: LayoutTemplate[] = [
  ...
  
  {
    ...
    description: 'Pinterest-style layout for diverse content and creative portfolios',
    category: 'minimal',
  ...
  
  {
    ...
    description: 'Clean masonry layout with generous spacing for artistic content',
    category: 'minimal',
...
export function getTemplateCategories(): string[] {
  const categories = new Set(CORE_TEMPLATES.map(template => template.category));
 ...
 */
export function getRecommendedTemplates(contentType: 'product' | 'photo' | 'video' | 'mixed'): LayoutTemplate[] {
  const recommendations: Record<typeof contentType, string[]> = {
    product: ['classic-product-grid', 'featured-carousel', 'mobile-swipe-gallery'],
