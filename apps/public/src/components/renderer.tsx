...
function CategorySection({ category, className }: CategorySectionProps) {
  ...
  const [categoryType, categoryTypeDetails] = category.categoryType;
  ...
  const getGridClass = () => {
    ...
    switch (itemsPerRow) {
      ...
      case 5: return 'grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3';
      case 6: return 'grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3';
      default: return 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4';
    }
  };
...
function CardRenderer({ category, displayType, className }: CardRendererProps) {
  ...
  const cardClass = cn(
    displayType === 'slider' && 'flex-none snap-start',
    className
  );
  ...
  switch (cardType) {
    ...
    case 'product':
      ...
      if (!product) {
        return (
          <div className={cn("aspect-square bg-muted rounded-lg flex items-center justify-center", cardClass)}>
