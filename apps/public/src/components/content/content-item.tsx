    ...
    <div className={`relative aspect-square overflow-hidden bg-gray-800 group ${className}`}>
      ...
      {overlay && (
        ...
        >
          <span
            className={
              overlay.position === "center" ? "bg-black bg-opacity-50 px-4 py-2 rounded" : ""
            }
          >
            {overlay.text}
          </span>
      ...
      {(title || price) && (
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-3">
          {title && <h3 className="text-white text-sm font-medium leading-tight">{title}</h3>}
          {price && <p className="text-white/90 text-xs mt-1">{price}</p>}
        </div>
      )}
