...
import { Upload, X, Image as ImageIcon, Loader, AlertCircle, CheckCircle, Pause, Play } from 'lucide-react';
            ...
            <div className="absolute top-2 right-2 bg-black bg-opacity-70 text-white text-xs px-2 py-1 rounded">
              {uploadState.result.compressed && (
                <div className="flex items-center gap-1">
                  ...
                  {((uploadState.result.originalSize! - uploadState.result.size) / uploadState.result.originalSize! * 100).toFixed(0)}% smaller
                </div>
              )}
            </div>
          )}
        ...
        <div className="flex flex-col items-center space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {uploadState.status === 'idle' && `Drag and drop or click to browse (max ${maxSize}MB)`}
          ...
          {uploadState.status === 'uploading' && uploadState.progress && (
            <div className="w-full max-w-xs">
              ...
              <div className="flex justify-between text-xs text-gray-500 mt-1">
                ...
                <span>
                  {formatBytes(uploadState.progress.loaded)} / {formatBytes(uploadState.progress.total)}
                </span>
              </div>
              {uploadState.progress.speed && uploadState.progress.remainingTime && (
                <div className="text-xs text-gray-500 mt-1">
                  {formatBytes(uploadState.progress.speed)}/s • {formatTime(uploadState.progress.remainingTime)} remaining
                </div>
              )}
            </div>
          )}
