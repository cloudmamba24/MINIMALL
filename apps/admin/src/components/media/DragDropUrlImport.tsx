...
import { Banner, Button, Card, Icon, Spinner, Text } from "@shopify/polaris";
import { DeleteIcon, ImportIcon, LinkIcon } from "@shopify/polaris-icons";
...
export function DragDropUrlImport({
      ...
      if (onValidateUrl) {
        for (const entry of urlEntries) {
          if (entry.status === "pending") {
            setUrls((prev) =>
              prev.map((u) => (u.id === entry.id ? { ...u, status: "validating" } : u))
            );
  ...
  const handleSubmit = useCallback(() => {
    const validUrls = urls
      .filter((u) => u.status === "valid" || (u.status === "pending" && !onValidateUrl))
      .map((u) => u.url);
  ...
  const validUrlCount = urls.filter(
    (u) => u.status === "valid" || (u.status === "pending" && !onValidateUrl)
  ).length;
                ...
                {urls.map((urlEntry) => (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                      {urlEntry.status === "validating" && <Spinner size="small" />}
