...
import { LayoutPreset, LAYOUT_PRESETS, SocialLayoutPreset, SOCIAL_LAYOUT_PRESETS } from "@minimall/core";
    ...
    preview: (
      <div className="grid grid-cols-3 gap-0.5 w-full h-8">
        {Array.from({ length: 9 }).map((_, i) => (
          <div key={i} className="bg-gradient-to-br from-purple-400 to-pink-400 rounded-sm aspect-square" />
        ))}
      </div>
    ),
    ...
    preview: (
      <div className="flex gap-0.5 w-full h-8">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="bg-gradient-to-t from-orange-400 to-yellow-300 rounded-full w-6 h-6 border-2 border-white flex-shrink-0" />
        ))}
      </div>
    ),
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      {showSocialPresets && (
        <div>
          ...
          <div className="space-y-3">
            {SOCIAL_LAYOUT_PRESETS.map(renderPreset)}
          </div>
        </div>
      )}
      ...
      <div>
        ...
        <div className="space-y-3">
          {LAYOUT_PRESETS.map(renderPreset)}
        </div>
      </div>
    </div>
  );
